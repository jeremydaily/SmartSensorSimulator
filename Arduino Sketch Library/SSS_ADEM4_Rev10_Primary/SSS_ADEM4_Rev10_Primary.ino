...
//  Wire.onReceive(buildCANMessage); //see request Event at the bottom of the code
  ...
  pinMode(HCrelay1Pin,OUTPUT);
  pinMode(HCrelay2Pin,OUTPUT);
  ...
  digitalWrite(HCrelay1Pin, LOW);
  digitalWrite(HCrelay2Pin, LOW);
...
if (relayState){
    ...
    if ((currentMillis - previousMillis100) > 99) { // Do this every 100 milliseconds
        ...
        CAN0.sendMsgBuf(0x18FEF128, 1, 8, stmp); //CCVS from SA=23
        CAN0.sendMsgBuf(0x18FEF121, 1, 8, stmp); //CCVS from SA=33
        CAN0.sendMsgBuf(0x18FEF131, 1, 8, stmp); //CCVS from SA=49
        CAN0.sendMsgBuf(0x18E00017, 1, 8, stmp); //Cab Message 1 CM1  from SA=25
        CAN0.sendMsgBuf(0x18E00021, 1, 8, stmp); //Cab Message 1 CM1  from SA=25
        CAN0.sendMsgBuf(0x18E00028, 1, 8, stmp); //Cab Message 1 CM1  from SA=25
        CAN0.sendMsgBuf(0x18E00031, 1, 8, stmp); //Cab Message 1 CM1  from SA=49
        
      }
      
    if(currentMillis - previousMillis10 > 19) {
