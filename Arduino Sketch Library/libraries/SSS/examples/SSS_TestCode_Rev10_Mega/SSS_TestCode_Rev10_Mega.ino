...
{ 
  sss.begin();
  sss.IDstring = "SYNER*SSS-DDECX*1R90006     "; //Change this to match the case of the SSS you are programming. Be sure to include trailing spaces.
...
void loop(){
  ...
  for (int g = 0; g < 20; g++) sss.settings[g]=wiperSetting;
  wiperSetting+=1;
  if (wiperSetting >100) wiperSetting=0;
  vOutSetting+=100;
  for (int g = 20; g < 24; g++) sss.settings[g]=vOutSetting;
  if (vOutSetting >5000) vOutSetting=0;
  sss.settings[24]=wiperSetting;
  
  for (int h = 0; h < 25; h++) adjustSetting(h);
  
  if (millis()-previousMillis1000 > 3000){
    for (int g = 26; g < 58; g++) sss.settings[g]=switchState; 
    for (int g = 78; g < 84; g++) sss.settings[g]=switchState; 
    switchState = !switchState;
    for (int h = 26; h < 58; h++) adjustSetting(h);
    for (int h = 78; h < 84; h++) adjustSetting(h);
